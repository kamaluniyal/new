    ...
    for (var i=2 ;i<=usedRowNum;i++){
        ...
        Promise.all([
            ...
            switch(values[3]){
                ...
                break; 
                case "DELETE":
                this.fnDelete(values[0].text,values[1],values[2],values[4],values[5],values[6],values[7]);
                break; 
...
//    } 
    ...
    if(validationParams[0]==="NA")
    {
        var validationFlag = false ;
    }

    if(responseParams[0]==="NA")
    {
...
APIlib.prototype.fnDelete = function (apiserver,APIPath,Authnetication,Inputs,expctdStatus,responseParams,validationParams) {
var promDefer = webdriver.promise.defer();   
